...
<section class="content">
    ...
    <div class="settings-title">
        ...
        <span class="online" style="display: inline-flex; align-items: center; gap: 6px;">
            <span style="width: 8px; height: 8px; border-radius: 50%; background: currentColor; display: inline-block;"></span> {{ ucfirst($device->status ?? 'Online') }}
        </span>
    </div>
            ...
            <div class="threshold-grid">
                ...
                <div class="threshold">
                    <div class="threshold-head">
                        ...
                        <span class="threshold-value" id="badge-voltage">{{ $threshold->max_voltage ?? 245 }}V</span>
                    </div>

                    <input type="range" name="max_voltage" min="180" max="270" step="1"
                           value="{{ old('max_voltage', $threshold->max_voltage ?? 245) }}"
                ...
                <div class="threshold">
                    <div class="threshold-head">
                        ...
                        <span class="threshold-value" id="badge-current">{{ $threshold->max_current ?? 15.5 }}A</span>
                    </div>

                    <input type="range" name="max_current" min="1" max="25" step="0.5"
                           value="{{ old('max_current', $threshold->max_current ?? 15.5) }}"
                ...
                <div class="threshold">
                    <div class="threshold-head">
                        ...
                        <span class="threshold-value" id="badge-temperature">{{ $threshold->max_temperature ?? 65 }}°C</span>
                    </div>

                    <input type="range" name="max_temperature" min="40" max="100" step="1"
                           value="{{ old('max_temperature', $threshold->max_temperature ?? 65) }}"
                ...
                <div class="threshold">
                    <div class="threshold-head">
                        ...
                        <span class="threshold-value" id="badge-smoke">{{ $threshold->max_smoke_ppm ?? 995 }} ppm</span>
                    </div>

                    <input type="range" name="max_smoke_ppm" min="200" max="2000" step="25"
                           value="{{ old('max_smoke_ppm', $threshold->max_smoke_ppm ?? 995) }}"
    ...
    <div class="panel network" style="margin-top: 20px;">
        ...
        <form method="POST" action="{{ route('settings.update') }}">
            @csrf
            <input type="hidden" name="max_voltage" value="{{ $threshold->max_voltage ?? 245 }}">
            <input type="hidden" name="max_current" value="{{ $threshold->max_current ?? 15.5 }}">
            <input type="hidden" name="max_temperature" value="{{ $threshold->max_temperature ?? 65 }}">
            <input type="hidden" name="max_smoke_ppm" value="{{ $threshold->max_smoke_ppm ?? 995 }}">
            ...
            <div style="display: flex; align-items: flex-end; gap: 12px; flex-wrap: wrap;">
                <div style="flex: 1; min-width: 220px;">
                    ...
                    <input type="number" name="kwh_rate" id="kwh_rate" step="0.01" min="0" max="100000"
                           value="{{ old('kwh_rate', $threshold->kwh_rate ?? 1444.70) }}"
    ...
    <div class="panel network" style="margin-top: 20px;">
        ...
        <div class="network-row">
            ...
            <strong>{{ $device->device_uid ?? 'ESP32_SOCKET_01' }}</strong>
        </div>
        ...
        <div class="network-row">
            ...
            <strong>v{{ $device->firmware_version ?? '2.1.4' }}</strong>
        </div>
        ...
        <button type="button" onclick="alert('Broker: HiveMQ Cloud TLS (Port 8883) aktif terhubung.')" class="wifi-btn" style="cursor: pointer;">
